...
export default function ProjectSection({ data }) {
  ...
  if (isMobile) {
    return (
      <section id="kegiatan" className="w-full py-10! relative border-b border-(--line)">
            ...
            {data.list.map((item, index) => (
              <SwiperSlide key={index}>
                <div className="w-60 h-60 m-auto! overflow-hidden border border-(--line) rounded-2xl p-3!">
                  {item.img == "" ? <div className="w-full h-full rounded-xl bg-cyan-200">{item.name}</div> : <img src={item.img} className="w-full rounded-xl" />}
                </div>
              </SwiperSlide>
            ))}
    ...
    return (
      <section id="kegiatan" className="w-full h-[400vh] relative flex justify-center border-b border-(--line)">
        <Line />
        ...
        <div className="w-[40%] h-full flex flex-col">
          {data.list.map((item, index) => (
            <div key={index} className="w-80 h-80 overflow-hidden m-auto! border border-(--line) rounded-3xl p-3!">
              {item.img == "" ? <div className="w-full h-full rounded-2xl bg-cyan-200">{item.name}</div> : <img src={item.img} className="w-full rounded-2xl" />}
            </div>
          ))}
        </div>
      </section>
