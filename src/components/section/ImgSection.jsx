    ...
    return (
      <div ref={sectionRef} className={`w-full ${isMobile ? "h-[70vh]" : "h-screen"}`}>
        <div className={`relative w-full ${isMobile ? "h-full" : "h-screen"} overflow-hidden`}>
          <img
            src={img}
            className={`absolute w-full -translate-y-1/2 ${isMobile ? "h-screen" : "h-[200vh]"} ${ts ? "opacity-20 grayscale" : "opacity-50"}`}
            alt="test"
            style={{
          ...
          {isMobile && (
            <div id="fasilitas" onMouseEnter={() => setTs(true)} onMouseLeave={() => setTs(false)} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          ...
          {isDesktop && (
            <div id="fasilitas" className="flex w-full h-full">
